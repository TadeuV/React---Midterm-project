...
export default function PokeCard ({name,index,category,tp2}) {
    ...
    return(
        <div className="card" onClick={handleDetailLink}>
            ...
            <div className="pokethumb">
                <img src={`/pokepics/${padLeft(index,3)}-${firstLetter(name)}.png`} alt={name}/>
            </div>
            <div className="typeWrapper">
                <div className="category" style={switchcolor(category)}>{firstLetter(category)}</div>
                {tp2 != "" ?  <div className="category2" style={switchcolor(tp2)}>{firstLetter(tp2)}</div>:""}
            </div>
        </div>
    );
